...
fn main() {
    ...
    let instructions: Vec<(Direction, u64)> = input
        .lines()
        .map(|line| {
            let parts = line.split(' ').collect::<Vec<&str>>();
            let instruction = parts[2].replace('(', "").replace(')', "").replace('#', "");
            let number =
                u64::from_str_radix(&instruction[0..5], 16).expect("Parsing a valid hex digit");
