...
fn get_neighbors(points: &[Vec<u32>], node: Node) -> Vec<Node> {
    ...
    if row + 1 < points.len() as u32 && node.direction != Direction::Up {
    ...
    if col + 1 < points[0].len() as u32 && node.direction != Direction::Left {
    ...
    if col > 0 && node.direction != Direction::Right {
